-- Fix forma_pagamento in fn_materialize_rule to use valid value
-- The check constraint only allows: dinheiro, pix, cartao_debito, cartao_credito, transferencia, outro
-- Change 'a_definir' to 'outro' which is a valid value
  ...
  WHILE v_current_date <= v_date_to LOOP
    ...
    IF v_weekday = ANY(v_rule.weekdays) THEN
      ...
      IF v_weeks_diff % v_rule.interval_weeks = 0 THEN
        ...
        -- Upsert appointment (Changed 'a_definir' to 'outro' to match check constraint)
        ...
        VALUES (
          ...
          v_rule.title, COALESCE(v_rule.amount, 0), 'outro', 'agendado', 'Gerado automaticamente'
        )
